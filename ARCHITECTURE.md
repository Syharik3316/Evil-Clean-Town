...
**Логотип и брендинг**: в шапке — текстовый лого «GoodWill · ORBITAL VIEW»
...
пережат из `newfront/assets/satellite-trim.png` (4.7 МБ → 200 КБ). Favicon —
статический `frontend/favicon.ico`, подключён `<link rel="icon">` на каждой странице
(nginx отдаёт его как обычный файл из корня `frontend/`).
...
кружок с первой буквой имени, как раньше (`.avatar-box`/`avatarBox()` в `js/profile.js`).
«Обо мне» — `bio` на `User` (волонтёр/организатор) и отдельно на `Organization` (карточка
организации в профиле организатора, редактируется через `PATCH /organizations/me`, только
своя организация; статус — `orgStatusBadge()`). Раздел «Аккаунт» в `profile.js`
(`accountSettingsHtml`/`bindAccountSettings`, общий для всех трёх ролей) даёт сменить
логин/пароль/email — email меняется в два шага (`POST /users/me/email/change` → код на
новый адрес → `POST /users/me/email/confirm`), логин/пароль — сразу, но оба требуют
повторного ввода текущего пароля.
